...
describe('copyVariantOverwriteAllCommand — chế độ (a) ghi đè toàn bộ', () => {
  it('đích trống → tạo mới toàn bộ theo nguồn (scale toạ độ, set syncRef), COPY background từ nguồn (kể cả undefined)', () => {
    const history = new HistoryStack();
    const source = textItem('s1', { syncKey: 'kS', content: 'Nguồn', box: { x: 100, y: 50, w: 200, h: 60 } });
    let state = createInitialState(
      docWith([
        { aspectId: '16:9', w: 16, h: 9, items: [source] }, // nguồn KHÔNG có background
    ...
    // Nguồn không có background → đích cũng thành undefined SAU KHI COPY (không phải "xoá" như
    // trước đây — chốt 2026-07-18: background copy TOÀN BỘ theo nguồn trong mọi chế độ).
    expect(target.background).toBeUndefined();
    // aspect/refW/refH của đích KHÔNG đổi.
    expect(target.aspect.id).toBe('4:3');
  });

  it('nguồn CÓ background → đích nhận ĐÚNG background của nguồn (kể cả ảnh) — chốt 2026-07-18: "copy = clone toàn bộ layout"', () => {
    const history = new HistoryStack();
    const source = textItem('s1', { syncKey: 'kS' });
    let state = createInitialState(
      docWith([
        { aspectId: '16:9', w: 16, h: 9, items: [source], background: { kind: 'image', src: 'bg.jpg' } },
        { aspectId: '4:3', w: 4, h: 3, items: [], background: { kind: 'color', color: '#000' } },
      ]),
    );
    ...
    const target = state.doc.variants[1]!;
    expect(target.background).toEqual({ kind: 'image', src: 'bg.jpg' });
  });
...
describe('copyVariantAddMissingCommand — chế độ (b) chỉ thêm cái thiếu', () => {
  ...
  it('đích CHƯA có background, nguồn CÓ → copy background của nguồn sang đích (chốt 2026-07-18: "chỉ thêm cái chưa có" áp dụng cả cho background)', () => {
    const history = new HistoryStack();
    let state = createInitialState(
      docWith([
        { aspectId: '16:9', w: 16, h: 9, items: [], background: { kind: 'color', color: '#123' } },
        { aspectId: '4:3', w: 4, h: 3, items: [] }, // đích chưa có background
      ]),
    );

    state = history.execute(copyVariantAddMissingCommand('16:9', '4:3'), state);

    expect(state.doc.variants[1]!.background).toEqual({ kind: 'color', color: '#123' });
  });

  it('đích ĐÃ có background riêng → GIỮ NGUYÊN, không bị ghi đè bởi nguồn', () => {
    const history = new HistoryStack();
    let state = createInitialState(
      docWith([
        { aspectId: '16:9', w: 16, h: 9, items: [], background: { kind: 'color', color: '#123' } },
        { aspectId: '4:3', w: 4, h: 3, items: [], background: { kind: 'color', color: '#456' } },
      ]),
    );

    state = history.execute(copyVariantAddMissingCommand('16:9', '4:3'), state);

    expect(state.doc.variants[1]!.background).toEqual({ kind: 'color', color: '#456' });
  });

  it('undo khôi phục đúng background cũ (kể cả undefined) sau khi background bị thêm mới', () => {
    const history = new HistoryStack();
    let state = createInitialState(
      docWith([
        { aspectId: '16:9', w: 16, h: 9, items: [], background: { kind: 'color', color: '#123' } },
    ...
    state = history.undo(state);
    expect(state.doc.variants[1]!.background).toBeUndefined();
  });
});
...
describe('copyVariantOverwriteExistingCommand — chế độ (c) ghi đè nội dung cho cái đã có', () => {
  ...
  it('đích ĐÃ có background → ghi đè bằng background của nguồn (chốt 2026-07-18: "cập nhật cái đã có" áp dụng cả cho background)', () => {
    const history = new HistoryStack();
    let state = createInitialState(
      docWith([
        { aspectId: '16:9', w: 16, h: 9, items: [], background: { kind: 'color', color: '#123' } },
  ...
  it('đích CHƯA có background → KHÔNG tự tạo mới (dù nguồn có), giữ undefined', () => {
    const history = new HistoryStack();
    let state = createInitialState(
      docWith([
        { aspectId: '16:9', w: 16, h: 9, items: [], background: { kind: 'color', color: '#123' } },
  ...
  it('undo khôi phục đúng background cũ của đích', () => {
    const history = new HistoryStack();
    let state = createInitialState(
      docWith([
        { aspectId: '16:9', w: 16, h: 9, items: [], background: { kind: 'color', color: '#123' } },
