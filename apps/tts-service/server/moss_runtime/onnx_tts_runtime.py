...
# TORCH-FREE PATCH: bỏ torch/torchaudio. Đọc + resample ref audio bằng soundfile+soxr
# (chỉ dùng ở _load_reference_audio). Core inference vốn đã torch-free.
...
class OnnxTtsRuntime(OrtCpuRuntime):
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        # TORCH-FREE: soundfile trả (samples, channels) → chuyển sang (channels, samples).
        ...
        if sample_rate != target_sample_rate:
            waveform = np.stack([
                _soxr.resample(waveform[c], sample_rate, target_sample_rate)
        ...
        # (1, channels, samples) float32 — giống output cũ (unsqueeze(0)).
