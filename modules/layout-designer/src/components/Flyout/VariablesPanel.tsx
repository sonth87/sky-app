  ...
  return (
    <>
      ...
      {keys.length === 0 ? (
        ...
        <div className="px-[14px] pb-[14px] overflow-y-auto flex-1 flex flex-col gap-[5px]">
          {keys.map((key) => (
            <div
              key={key}
              onMouseDown={onSpawnDown({ kind: 'var', key, label: `@${key}` })}
              className="shrink-0 flex items-center gap-[7px] border border-[#eceef2] rounded-lg p-[7px_9px] cursor-grab bg-[#fcfcfd]"
            >
