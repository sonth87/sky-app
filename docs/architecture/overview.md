...
>
> Kế hoạch gốc lập lúc khởi tạo repo (2026-07-11) và nhật ký triển khai từng giai đoạn: [`docs/roadmap/plans/platform-architecture-ga1-7.md`](../roadmap/plans/platform-architecture-ga1-7.md) (kế hoạch, trạng thái `done`) · [`docs/dev/history/`](../dev/history/README.md) (nhật ký theo ngày). Các kế hoạch đang chờ triển khai: [`docs/roadmap/`](../roadmap/).
...
Kế hoạch gốc đầy đủ (bối cảnh, lựa chọn đã cân nhắc, rủi ro dự kiến lúc lập): [`docs/roadmap/plans/platform-architecture-ga1-7.md`](../roadmap/plans/platform-architecture-ga1-7.md). Nhật ký chi tiết từng giai đoạn theo ngày: [`docs/dev/history/`](../dev/history/README.md).
...
**Đã chốt:** ports&adapters · entitlement Ed25519 offline · monorepo pnpm+Turbo · React 19/shadcn/Tailwind v4/TanStack · **KHÔNG Module Federation** (offline-first → static bundle) · device-layout giữ repo riêng, tích hợp dạng **git dependency** (`github:sonth87/device-layout#<tag>`, `dist-lib/` commit sẵn — xem §6 bên dưới).
...
**Còn mở:** backend web (reuse tts-service hay riêng?) · hạ tầng cấp phát license key.

**Chốt: publish/version device-layout lib qua git dependency (2026-07-15).** sky-app cài `@sonth87/device-layout` qua `"github:sonth87/device-layout#v0.1.0"` (4 chỗ: `apps/shell-electron`, `apps/shell-web`, `packages/device-shell`, `modules/ceremony`). Bản build Vite lib (`dist-lib/`, ~1.8MB) được **commit vào git** repo device-layout, nên pnpm resolve trực tiếp không cần build — sky-app KHÔNG cài devDependencies Next.js (~583MB) của device-layout. Version tường minh qua **git tag** (`#v0.1.0`), muốn cập nhật thì device-layout build+commit dist-lib+tag mới, sky-app đổi số tag. Cách này thay cơ chế tarball local `file:.vendor/*.tgz` cũ (đã bỏ, cùng `scripts/vendor-device-layout.sh`) — tarball buộc mọi máy phải có sẵn checkout device-layout + chạy script thủ công. Chi tiết + so sánh phương án: [dev/history/2026-07-15-device-layout-git-dependency-commit-dist-lib.md](../dev/history/2026-07-15-device-layout-git-dependency-commit-dist-lib.md), quy trình cập nhật: [dev/tooling.md](../dev/tooling.md).

*Lịch sử quyết định chi tiết: [dev/history/](../dev/history/README.md).*
