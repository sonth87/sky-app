...
### Cú pháp ĐÃ CHỐT: `@var` — MỞ, không đóng đuôi (2026-07-16 vòng 2, đảo lại quyết định `{{key}}`)

> **Lịch sử quyết định:** `@key@` (cũ, đóng) → `{{key}}` (chốt sáng 2026-07-16 theo bản thiết
> kế mapping) → **`@var` mở** (chốt lại chiều 2026-07-16 theo yêu cầu Sonth). Lý do đảo: Sonth
> muốn **thân thiện + quen thuộc như tag Facebook** (`@tên`) — chỉ cần `@` + tên, không cần
> nhớ đóng ngoặc. "Sau biến là một tập hợp ký tự được ghi nhận là kết thúc, tiêu biểu nhất là
> dấu cách." Đồng thời cú pháp này **đồng bộ luôn với `renderTemplate` TTS hiện có** (vốn đã
> dùng `@key`) — gần như không phải đổi phía TTS, một lợi thế lớn.

```
@var               // MỞ — @ + tên biến, KHÔNG có ký tự đóng. Kết thúc khi gặp ký tự ngoài
                    // [a-zA-Z0-9_-] (dấu cách, dấu câu, chữ tiếng Việt có dấu, xuống dòng...)
```

**Ví dụ:**
```
"tên tôi là @full_name.xin chào!"   → biến: full_name   (dừng ở dấu ".")
"@chuc_vu @full_name"               → 2 biến: chuc_vu, full_name (dừng ở dấu cách)
"@ho-ten của bạn"                    → biến: ho-ten       (cho phép gạch ngang giữa)
```

### Quy tắc chính xác (để parser KHÔNG mơ hồ)

**1. `@` phải đứng sau KHOẢNG TRẮNG hoặc ĐẦU DÒNG mới được coi là biến.**
```
"@full_name"          → @full_name là biến (đầu dòng)
"chào @full_name"     → @full_name là biến (sau dấu cách)
"email a@b.com"       → @b KHÔNG phải biến (@ dính liền chữ "a" phía trước — là @ thường)
```
→ Giải quyết luôn "ký tự @ thật trong email/địa chỉ" mà KHÔNG cần escape: chỉ cần @ dính liền
ký tự trước thì nó là @ thường. Đúng lời Sonth: *"phía trước @ là khoảng trống"*.

**2. Tên biến: ký tự `[a-zA-Z0-9_-]`, BẮT ĐẦU và KẾT THÚC bằng chữ cái hoặc số (không phải dấu).**
```
Cho phép giữa:  chữ (không dấu), số, gạch dưới _, gạch ngang -
Bắt đầu:        chữ/số/_ (không bắt đầu bằng -)
Kết thúc:       chữ/số/_ (KHÔNG kết thúc bằng - hay _ ... thực ra _ ở cuối vẫn nhận; DẤU - ở
                cuối bị NHẢ RA) — theo lời Sonth: "không được kết thúc với dấu mà phải kết thúc
                với chữ cái hoặc số"
KHÔNG:          dấu tiếng Việt, khoảng trắng, ., {, }, #, @ ...
```
Ví dụ ranh giới: `"@ho-ten-"` → biến là `ho-ten` (dấu `-` cuối bị nhả); `"@a-1b"` → `a-1b`.

**3. Regex chốt:**
```
/(^|\s)@([a-zA-Z0-9_](?:[a-zA-Z0-9_-]*[a-zA-Z0-9_])?)/g
        └─ nhóm 1: @ đứng sau đầu dòng/khoảng trắng
                    └─ nhóm 2 (tên biến): bắt đầu bằng chữ/số/_, giữa cho phép -, kết thúc bằng chữ/số/_
```
Lưu ý implement: dùng lookbehind `(?<=^|\s)` nếu môi trường hỗ trợ, hoặc bắt `(^|\s)@...` rồi
giữ lại ký tự nhóm 1 khi replace.

### Namespace — KHÔNG "cấm trùng key hệ thống" (xem §2.5/§3)

> Layout tự do khai token, Global chỉ là DANH SÁCH GỢI Ý (không phải danh sách "đã đăng ký,
> cấm dùng lại"). Xem §2.5, §3.

### UI: chip đẹp + autocomplete (Sonth chốt "cả hai — cú pháp @ gọn + UI chip")

Cú pháp `@var` chỉ là **cách LƯU bên trong**. Trong editor, biến hiển thị thân thiện:
- **Gõ `@`** → hiện dropdown autocomplete (Global + `variable_registry` — §2.6, §4).
- Token đã chèn hiển thị dạng **chip màu** (không phải text thô `@full_name`) — người dùng thấy
  1 "viên" biến rõ ràng, click để đổi/xóa. Đây là phần "thân thiện" thật sự (file 23 editor-core
  quản lý item text với token → render chip khi ở chế độ sửa).
- Người dùng không chuyên (A6) không cần biết cú pháp `@` — chỉ bấm chèn từ dropdown.

### Thuộc tính biến dạng `@var:modifier`? — KHÔNG làm ở bản đầu

Format/transform nằm ở **định nghĩa biến** (CustomVariable/computed mapping), không rải rác
trong token. Giữ `@var` thuần cho regex ổn định. Bổ sung `:modifier` sau nếu thật cần (không
phá cú pháp cũ). Lưu ý: nếu sau này thêm công thức inline có phép trừ, phải viết `@a - @b`
(có dấu cách quanh `-`) để không lẫn với gạch ngang trong tên biến — ghi nhận, chưa cần lo giờ.

### Đồng bộ với `renderTemplate` TTS hiện có (lợi thế lớn của việc chọn `@var`)

`modules/ceremony/src/lib/renderTemplate.ts` hiện tại **đã dùng cú pháp `@key`** cho TTS template
(vd câu đọc "Xin chúc mừng @full_name"). Chọn `@var` cho layout nghĩa là **1 cú pháp DUY NHẤT**
cho cả layout, TTS, và custom variable — không phải migrate TTS sang cú pháp khác, không có 2 cú
pháp song song. Khi implement thật:
- Rà lại `renderTemplate` hiện tại xem regex ranh giới có **khớp đúng 4 quy tắc §1** không (đặc
  biệt quy tắc "@ sau khoảng trắng" và "kết thúc bằng chữ/số, nhả dấu `-`/`_` cuối") — nếu regex
  cũ lỏng hơn (VD không yêu cầu @ sau khoảng trắng), cần siết lại 1 lần cho khớp, dùng chung 1
  hàm parse token cho cả layout lẫn TTS (đặt ở `slide-shared`).
- Đây là việc nhỏ (siết regex + gom về 1 hàm), KHÔNG phải viết lại — khác hẳn kịch bản phải
  migrate toàn bộ TTS template sang `{{}}` nếu đã chọn cú pháp đóng. Đây là 1 lý do thực dụng
  ủng hộ quyết định `@var`.
...
```
Việc A — Layout TỰ ĐỊNH NGHĨA token của chính nó (chuỗi tự do sau dấu @)
...
`content`, không cần đặt token `@...`, nếu nó thực sự không đổi theo Event. Nếu cần đổi theo từng
...
- [x] Token: cú pháp `@var` — MỞ (không đóng đuôi), `@` sau khoảng trắng/đầu dòng, tên biến
      `[a-zA-Z0-9_-]` bắt đầu+kết thúc bằng chữ/số. ĐÃ CHỐT 2026-07-16 vòng 2 (đảo lại `{{key}}`),
      đồng bộ với `renderTemplate` TTS. Xem §1.
