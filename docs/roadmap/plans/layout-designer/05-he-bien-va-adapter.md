...
>
> ⚠️ **Đã cập nhật 2026-07-16:** (1) cú pháp token là **`@var` (mở, không đóng đuôi, sau khoảng
> trắng)** — quy định đầy đủ ở [09](09-quy-dinh-variable.md) §1. (2) `FieldMappingProfile.
> targetType` (union đóng `student|employee|generic`) đã đổi thành **`subjectType: string` mở**
> — khớp [11](11-canonical-da-loai-va-loop.md) Phần 1. (3) Danh sách biến quản lý ở đâu: bản
> dưới đây (§"Danh sách biến quản lý ở đâu") là bản CŨ — hướng ĐÚNG là "layout tự do khai token,
> Event map field", xem [09](09-quy-dinh-variable.md) §2.5 (thay thế).
...
export interface FieldMappingProfile {
  ...
  subjectType: string;           // ĐÃ ĐỔI 2026-07-16 (từ targetType union đóng) — mở, khớp
                                  ...
                                  // "group"|... (không union cứng, mở cho loại đối tượng mới)
  naturalKeyField: string;       // MỚI (file 22) — cột làm khóa tự nhiên định danh bản ghi,
                                  // để re-import ổn định id (VD "student_code", "manv")
